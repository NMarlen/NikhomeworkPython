# lesson_09/tests/test_students_sql.py
...
def test_insert_student(engine, cleanup):
    ...
    assert row["age"] == 20
    # В SQLite булево — это 0/1
    assert not bool(row["is_deleted"])
...
def test_soft_delete_student(engine, cleanup):
    with engine.begin() as conn:
        ...
        conn.execute(
            text("UPDATE test_students SET is_deleted = 1 WHERE id = :id"),
            {"id": new_id},
        )
